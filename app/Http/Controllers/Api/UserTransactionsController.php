...
class UserTransactionsController extends BaseController
{
	...
	public function getTransactions(Request $request)
	{
		...
		'taluka','address','city_town','district','state','pincode','latitude','longitude','created_at','updated_at',
            DB::raw('(select CONCAT("'.$url3.'", image_name) from  poultryhatchery_center_images where poultryhatchery_center_id  = poultryhatchery_centers.id order by id asc limit 1) as image_name'))->where('user_id', $user_id)->get();
		$poultryModuleArr = [];
		...
		'suppliers.longitude','suppliers.created_at','suppliers.updated_at',DB::raw('(select CONCAT("'.$url18.'", image_name) from  supplier_product_images where supplier_id  = suppliers.id order by id asc limit 1) as image_name'))
			->where('user_id', $user_id)->get();
		$supplierModuleArr = [];
