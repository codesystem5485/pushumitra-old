...
rateable_id  =   chemists.id AND module_id ='.$module_id.' ) as star_rating_count'));
	...
	public function deleteChemist(Request $request){
		$postData = request()->all();		
		$validator = Validator::make($postData, [
			'id' => 'required',
		]);
		...
		$id = $request->id;
        $result = Chemist::where('id',$id)->first();
		$name = '';
		if($result){
			$name = $result->shop_name;
			$images = ChemistShopImages::where('chemist_id',$result->id)->get();
        ...
        $response=[];
        ## Store log
        $message = trans('messages.chemist_delete',['name' => $name]);
        storeActicityLog(trans('messages.chemist_delete'),$message,$postData['user_id'],$result);
		return $this->sendResponse($response,$message,200);
    }
