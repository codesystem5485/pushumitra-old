    ...
    <div class="container-fluid">
        <div class="block-header">
            <div class="row">
                ...
                <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>{{ __('general.gresolutionfiles_list') }}</h2>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/dashboard')}}"><i class="icon-home"></i></a></li>
                    <li class="breadcrumb-item">{{ __('general.gresolutionfiles_list') }}</li>
                </ul>
                </div>
            </div>
        </div>
        <div class="row clearfix">
            <div class="col-lg-12">
                ...
                @include('backend.layouts.flash-message')
                    ...
                    @can('book-create')
                    <a href="{{$url['createUrl']}}" class="btn btn-info">{{ __('general.gresolutionfiles_add') }} </a>
                    @endcan
                </div>
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                            <thead>
                            <tr>
                                <th>{{ __('general.gresolutionfiles') }}</th>                                
                                <th>{{ __('general.action') }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($gresolutionfiles as $row)
                            <tr>
                                <td>{{$row->title}}</td>                                 
                                <td>
                                    @can('grfile-edit')
                                    <a href="{{route('gresolutionfiles.edit',['id' => $row->id])}}">
                                    ...
                                    @endcan
                                    @can('grfile-delete')
                                    <a href="{{route('gresolutionfiles.delete',['id' => $row->id])}}" onclick="return confirm('Do you really want to delete the record(s)?')">
