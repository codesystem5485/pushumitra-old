    ...
    <div class="container-fluid">
        <div class="block-header">
            <div class="row">
                ...
                <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>{{ __('general.animal_type_list') }}</h2>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/dashboard')}}"><i class="icon-home"></i></a></li>
                    <li class="breadcrumb-item">{{ __('general.animal_type_list') }}</li>
                </ul>
                </div>
            </div>
        </div>
        <div class="row clearfix">
            <div class="col-lg-12">
                ...
                @include('backend.layouts.flash-message')
                    ...
                    @can('animal-type-create')
                    <a href="{{$url['createUrl']}}" class="btn btn-info">{{ __('general.animal_type_add') }} </a>
                    @endcan
                </div>
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                            ...
                            <tr>
                                <td>{{$type->animal_type}}</td>                                 
                                <td>
                                    @can('animal-type-edit')
                                    <a href="{{route('animal.edit-type',['id' => $type->id])}}">
                                    ...
                                    @endcan
                                    @can('animal-type-delete')
                                    <a href="{{route('animal.delete-type',['id' => $type->id])}}" onclick="return confirm('Do you really want to delete the record(s)?')">
