    ...
    <div class="container-fluid">
        <div class="block-header">
            <div class="row">
                ...
                <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>{{ __('general.characteristics_list') }}</h2>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/dashboard')}}"><i class="icon-home"></i></a></li>
                    <li class="breadcrumb-item">{{ __('general.characteristics_list') }}</li>
                </ul>
                </div>
            </div>
        </div>
        <div class="row clearfix">
            <div class="col-lg-12">
                ...
                @include('backend.layouts.flash-message')
                    ...
                    @can('characteristics-create')
                    <a href="{{$url['createUrl']}}" class="btn btn-info">{{ __('general.characteristics_add') }} </a>
                    @endcan
                </div>
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                            <thead>
                            <tr>
                                <th>{{ __('general.characteristics') }}</th>                                
                                <th>{{ __('general.action') }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($characteristics as $type)
                            <tr>
                                <td>{{$type->characteristic}}</td>                                 
                                <td>
                                    @can('characteristics-edit')
                                    <a href="{{route('characteristics.edit',['id' => $type->id])}}">
                                    ...
                                    @endcan
                                    @can('characteristics-delete')
                                    <a href="{{route('characteristics.delete',['id' => $type->id])}}" onclick="return confirm('Do you really want to delete the record(s)?')">
