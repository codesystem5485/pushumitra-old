    ...
    <div class="container-fluid">
        <div class="block-header">
            <div class="row">
                ...
                <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>{{ __('general.product_list') }}</h2>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/dashboard')}}"><i class="icon-home"></i></a></li>
                    <li class="breadcrumb-item">{{ __('general.product_list') }}</li>
                </ul>
                </div>
            </div>
        </div>
        <div class="row clearfix">
            <div class="col-lg-12">
                ...
                @include('backend.layouts.flash-message')
                    ...
                    @can('product-create')
                    <a href="{{$url['createUrl']}}" class="btn btn-info">{{ __('general.product_add') }} </a>
                    @endcan
                </div>
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                            ...
                            <tr>
                                ...
                                <td>{{$sale->getProductOwner->first_name." ".$sale->getProductOwner->middle_name." ".$sale->getProductOwner->last_name}}</td>                                 
                                <td>{{$sale->mobile_number}}</td>                                 
                                <td>{{$sale->price}}</td>                                 
                                <td>{{$sale->address}}</td>                           
                                <td>
                                    @can('product-detail')
                                    <a href="{{route('add-product.detail',['id' => $sale->id])}}"><button class="btn btn-sm btn-icon btn-pure btn-default on-default button-view" data-toggle="tooltip" data-original-title="{{ __('general.edit') }}"><i class="icon-user" aria-hidden="true"></i> 
                                    </button></a>
                                    @endcan

                                    @can('product-edit')
                                    <a href="{{route('add-product.edit',['id' => $sale->id])}}">
                                    ...
                                    @can('product-delete')
                                    <a href="{{route('add-product.delete',['id' => $sale->id])}}" onclick="return confirm('Do you really want to delete the record(s)?')">
