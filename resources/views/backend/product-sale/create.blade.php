    ...
    <div class="container-fluid">
        <div class="block-header">
            <div class="row">
                <div class="col-lg-5 col-md-8 col-sm-12">
                    <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>@if(!empty($productsale))
                    {{ __('general.product-sale_edit') }}
                    @else
                    {{ __('general.product-sale_create') }}
                    @endif </h2>
                <ul class="breadcrumb"> 
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12">
                ...
                <form action="@if(empty($productsale)){{route('product-sale.store')}}@else{{route('product-sale.update',['id' => $productsale->id])}}@endif" method="post" enctype="multipart/form-data"> 
                    @csrf  
                <div class="body">
                    <div class="input-group mb-3">
                     ...
                     <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control"  aria-describedby="basic-addon3" name="contact_name_of_owner" value="@if(empty($productsale)){{old('contact_name_of_owner')}}@else{{$productsale->contact_name_of_owner}}@endif"placeholder="{{ __('general.enter_contact_name_of_owner') }}">
                    </div>
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control"  aria-describedby="basic-addon3" name="contact_number_of_owner" value="@if(empty($productsale)){{old('contact_number_of_owner')}}@else{{$productsale->contact_number_of_owner}}@endif"placeholder="{{ __('general.enter_contact_number_of_owner') }}">
                    </div>
					<div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <span class="input-group-text" >Email Id :</span>
                        </div>
                        <input type="email" class="form-control"  aria-describedby="basic-addon3" name="email_id" value="@if(empty($productsale)){{old('email_id')}}@else{{$productsale->email_id}}@endif"placeholder="Enter Email Id">
                    </div>
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <span class="input-group-text" >{{ __('general.price') }}* :</span>
                        </div>
                        <input type="text" class="form-control"  aria-describedby="basic-addon3" name="price" value="@if(empty($productsale)){{old('price')}}@else{{$productsale->price}}@endif"placeholder="{{ __('general.enter_price') }}">
                    </div>
                        ...
                        @if(count($productimages))
                            @foreach($productimages as $value)
                                <div class="input-group mb-2" style="align:left;">
                                    <img height="100" width="100" src="{{url("/upload/productsale/")}}/{{$value->image_name}}" />
...
<script>
   $(document).on('click',".removeProductImage",function(e){
        e.preventDefault();
        if(confirm("Do you really want to delete this product sale image?"))
        {
        var image_val = $(this).attr('image_val');
        var actionurl = webUrl+"/pashumitra/product-sale/"+image_val+"/remove";
         $.ajax({
            url: actionurl,
            type: "get",
