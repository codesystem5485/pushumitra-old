        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                ...
                @include('backend.layouts.flash-message')
                    ...
                    <a href="{{$url['createUrl']}}" class="btn btn-info">{{ __('general.product-sale_add') }} </a>
                    @endcan
                </div>
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                            ...
                            <tr>
                                ...
                                <td>{{$sale->address}}</td>                           
                                <td>
                                    @can('product-sale-detail')
                                    <a href="{{route('product-sale.detail',['id' => $sale->id])}}"><button class="btn btn-sm btn-icon btn-pure btn-default on-default button-view" data-toggle="tooltip" data-original-title="{{ __('general.edit') }}"><i class="icon-user" aria-hidden="true"></i> 
                                    </button></a>
                                    @endcan

                                    @can('product-sale-edit')
                                    <a href="{{route('product-sale.edit',['id' => $sale->id])}}">
                                    ...
                                    <a href="{{route('product-sale.delete',['id' => $sale->id])}}" onclick="return confirm('Do you really want to delete the record(s)?')">
